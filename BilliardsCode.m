...
% Start logging the output to diary file
...
for i=1: length(cue_velocities)
    ...
    % Save the start position as an image
    ...
    while(true)
        ...
        if (not(any_moving))
            % Render the last frame, regardless of divider
            for j=1:length(balls)
        ...
        for j=1:length(balls)
            ...
            if j+1 <= length(balls)
                for k=j+1:length(balls)
                    balls(j).compute_ball_collision(balls(k)); % Compute collision of balls
                end
            end
        end
        
        % Write frames to the video, subject to the frame divider
    ...
    % Save the final position of the balls as an image
